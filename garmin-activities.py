...
from garmin_session_auth import get_garmin_client
...
def activity_needs_update(existing_activity, new_activity):
    ...
    has_subactivity = (
        ...
        and existing_props["Subactivity Type"] is not None
        and existing_props["Subactivity Type"].get("select") is not None
    )
    ...
    return (
        ...
        or existing_props["Avg Pace"]["rich_text"][0]["text"]["content"]
        != format_pace(new_activity.get("averageSpeed", 0))
        or existing_props["Avg Power"]["number"] != round(new_activity.get("avgPower", 0), 1)
        or existing_props["Max Power"]["number"] != round(new_activity.get("maxPower", 0), 1)
        or existing_props["Training Effect"]["select"]["name"]
        != format_training_effect(new_activity.get("trainingEffectLabel", "Unknown"))
        or existing_props["Aerobic"]["number"]
        != round(new_activity.get("aerobicTrainingEffect", 0), 1)
        or existing_props["Aerobic Effect"]["select"]["name"]
        != format_training_message(new_activity.get("aerobicTrainingEffectMessage", "Unknown"))
        or existing_props["Anaerobic"]["number"]
        != round(new_activity.get("anaerobicTrainingEffect", 0), 1)
        or existing_props["Anaerobic Effect"]["select"]["name"]
        != format_training_message(new_activity.get("anaerobicTrainingEffectMessage", "Unknown"))
        or existing_props["PR"]["checkbox"] != new_activity.get("pr", False)
        or existing_props["Fav"]["checkbox"] != new_activity.get("favorite", False)
        or existing_props["Activity Type"]["select"]["name"] != activity_type
        or (
            has_subactivity
            and existing_props["Subactivity Type"]["select"]["name"] != activity_subtype
        )
        or (not has_subactivity)  # If the property doesn't exist, we need an update
    )
...
def create_activity(client, database_id, activity):
    ...
    properties = {
        ...
        "Avg Pace": {
            "rich_text": [{"text": {"content": format_pace(activity.get("averageSpeed", 0))}}]
        },
        "Avg Power": {"number": round(activity.get("avgPower", 0), 1)},
        "Max Power": {"number": round(activity.get("maxPower", 0), 1)},
        "Training Effect": {
            "select": {
                "name": format_training_effect(activity.get("trainingEffectLabel", "Unknown"))
            }
        },
        "Aerobic": {"number": round(activity.get("aerobicTrainingEffect", 0), 1)},
        "Aerobic Effect": {
            "select": {
                "name": format_training_message(
...
def update_activity(client, existing_activity, new_activity):
    ...
    properties = {
        ...
        "Avg Pace": {
            "rich_text": [{"text": {"content": format_pace(new_activity.get("averageSpeed", 0))}}]
        },
        "Avg Power": {"number": round(new_activity.get("avgPower", 0), 1)},
        "Max Power": {"number": round(new_activity.get("maxPower", 0), 1)},
        "Training Effect": {
            "select": {
                "name": format_training_effect(new_activity.get("trainingEffectLabel", "Unknown"))
            }
        },
...
def main():
    ...
    # Initialize Garmin client with session authentication
    # This uses saved session if available, otherwise falls back to credentials
